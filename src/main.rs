...
use midi_input::{MidiInput, MidiInputPlugin};
...
fn select_device(mut device_events: EventReader<SelectDeviceEvent>, midi_input: Res<MidiInput>) {
    ...
    if !device_events.is_empty() {
        for device_event in device_events.iter() {
            ...
            midi_input.connect(midi_input.ports[*device_id].1.clone());
        }
    }
}
...
//                                 };
//                                 // event_system_writer.send(MidiInputKey {
//                                 //     event: event_type,
//                                 //     id: message[1],
//                                 //     intensity: message[2],
//                                 // });
...
fn select_device_ui(
    ...
    egui::Window::new("Select a MIDI device").show(context, |ui| {
        let ports = midi_input.ports.iter().enumerate();
        for (index, (device_name, port)) in ports {
            // let device_name = midi_state.input.port_name(port).unwrap();
            if ui.button(device_name.clone()).clicked() {
...
fn sync_device_input(mut input_events: EventReader<MidiInputKey>) {
    if !input_events.is_empty() {
        for input_event in input_events.iter() {
            println!("Key #: {}, State: {:?}", input_event.id, input_event.event);
        }
    }
}
...
//     device_input: NonSend<MidiInputConnection<()>>,
// ) {
//     let context = contexts.ctx_mut();
//     egui::Window::new("Select a MIDI device").show(context, |ui| {
